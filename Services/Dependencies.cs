...
namespace Package.Helper.Services
{
    public class Dependencies : IDependencies
    {
                ...
                foreach (var project in dependencyGraph.Projects)
                {
                    foreach (var targetFramework in project.TargetFrameworks)
                    {
                        foreach (var dependency in targetFramework.Dependencies)
                        {
                            ...
                            {
                                packageDict[packageName] = packageVersion;
                            }
                            else if (Versioning.IsNewSemverLarger(packageDict[packageName], packageVersion))
                            {
                                packageDict[packageName] = packageVersion;
                            }
                        }
        ...
        private static void PackageVersionPrinter(IReadOnlyDictionary<string, string> data)
        {
            ...
            {
                Console.WriteLine($"{packageName.PadRight(maxLength)}     {data[packageName]}");
            }
        }
    }
